...
4. **Run the full repo test suite.** Use the repo's documented runner
   ...
   fix.
5. **Second blast-radius scan.** Re-search the codebase for the same anti-pattern
   from `4c-cause`. Fix any new hits in scope, or list remaining ones in the
   dossier.
6. **Harden against the bug class** so it cannot reoccur silently: a type
   constraint, a schema check, a lint rule, an invariant assertion, or a guard —
   whichever fits.
7. **Optional quality-scan gate.** If a scanner is available, run a code-quality
...
8. **Final report** in the dossier Check section (mirrored in the commit message
